    ...
    this.each(function() {
      ...
      <div class="imageuploadify well">
        ...
        <div class="imageuploadify-images-list text-center">
          ...
          <span class=''>Drag & Drop Your Files</span><br>
          <span class=''>or</span>
          <button type="button" class="btn btn-default" style="color:blue;background-color:white">Select file to upload</button>
        </div>
      </div>
      `);
      ...
      const readingFile = (id, file) => {
        ...
        let container = $(`<div class='imageuploadify-container'>
          ...
          <div class='imageuploadify-details' style="font-size: 12px">
            <span>${file.name}</span>
            <span>${file.type}</span>
            <span>${file.size}</span>
          </div>
        ...
        if (file.type && file.type.search(/image/) != -1) {
          ...
          let type = "<i class='fa fa-file'></i>";
           // If the file is an PDF file, replace the icon by an PDF file icon.
          if (file.type.search(/pdf/) != -1) {
            type = "<i class='fa fa-file-pdf' style='color:red'></i>";
          }
      ...
      $(self).closest("form").validate({
        ...
        xhr.open("POST",$('#note_files').attr("action"), true);
        console.log($('#note_files').attr("action"))
        xhr.send(formData);
